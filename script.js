const works = [{
    ...
    id: 33,
    type: 'Visual Identities',
    title: 'Haifa Indpt Film Festival',
    year: 2019,
...
const editorialDesigns = works.filter(work => work.type.trim() === 'Editorial Design');
const visualIdentities = works.filter(work => work.type.trim() === 'Visual Identities');
const postersArtworks = works.filter(work => work.type.trim() === 'Posters and Artworks');

// Function to generate works dynamically
function generateWorks(array, elemntID) {
    let HTMLcontainer = `<p>${array[0].type}</p>`;
    array.forEach((item) => {
        HTMLcontainer +=
            `<div class="workLink" data-id="${item.id}"><p>${item.year}</p><p>${item.title}</p></div>`;
    });
    document.getElementById(`${elemntID}`).innerHTML = HTMLcontainer;
}

// Call the function for each section
generateWorks(visualIdentities, 'visualIdentities');
generateWorks(editorialDesigns, 'editorialDesign');
generateWorks(postersArtworks, 'postersArtworks');

// Add icon
document.getElementById('postersArtworks').innerHTML +=
    `<div class="space"><p>
    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
    viewBox="0 0 32 32" height="32" width="32" part="svg"
    draggable="true"><path part="base-color" fill="currentColor"
    d="M 4.84 9.18 C 4.84 14.55 6.97 17.08 12.36 17.08 L 
    20.40 17.08 L 23.54 16.99 L 21.00 19.30 L 16.48 23.84 
    L 17.93 25.27 L 27.16 16.05 L 17.93 6.82 L 16.47 8.27
    L 21.00 12.78 L 23.55 15.11 L 20.40 15.01 L 12.47 15.01
    C 8.27 15.01 6.87 13.20 6.87 9.07 L 6.87 6.73 L 4.86 6.73
    L 4.84 9.18 Z"></path></svg><p>Space</p>
    </div>`

// Event listener for Space
document.querySelector('.space').addEventListener('click', () => {
    document.querySelector('.right').innerHTML = space.html;
    document.querySelector('.right').classList.add('unGrid')
})

// Event listener for the generated links
document.querySelectorAll('.workLink').forEach((work) => {
    work.addEventListener('click', () => {
        document.querySelector('.right').classList.remove('unGrid')
        const workId = Number(work.dataset.id);
        ...
        document.querySelector('.right').innerHTML = contentDiv;
    });
});
...
setInterval(function () {
    ...
    if (currentImage === 1) {
        ...
        img2.classList.add('visible'); // Fade in img2
        img1.classList.remove('visible'); // Fade out img1
        currentImage = 2;
    } else {
        img1.src = nextImgSrc; // Load new image into img1
        img1.classList.add('visible'); // Fade in img1
        img2.classList.remove('visible'); // Fade out img2
        currentImage = 1;
    }
...
}, 100); // 100ms interval for image change
